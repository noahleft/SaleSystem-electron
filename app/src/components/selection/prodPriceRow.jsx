...
class ProdPriceRow extends React.Component {
  constructor(props) {
    super(props);
    this.state = {value: ''};
  ...
  handleSubmit(e) {
    if (this.state.value != '') {
      let CR = {COMP_ID: Number(e.target.formProd.value),
                PROD_ID: this.props.priceManager.selectedProdID,
  ...
  render() {
    ...
    onChange={this.handleChange} disabled={this.props.disabled} comp_id={display.ID}/>
    </Form.Group>
    </Form></td>
    </tr>
    );
...
export default connect(mapStateToProps, mapDispatch)(ProdPriceRow);
